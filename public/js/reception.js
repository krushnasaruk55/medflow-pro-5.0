...
function loadDepartments() {
  fetch(`${API_BASE}/api/departments`, { credentials: 'include' }).then(r => r.json()).then(list => {
...
function loadDoctors(dept) {
  ...
  fetch(url, { credentials: 'include' }).then(r => r.json()).then(list => {
    allDoctors = list;
...
function loadPatients() {
  fetch(`${API_BASE}/api/patients`, { credentials: 'include' }).then(r => r.json()).then(list => {
    patientsList = list;
    renderTable();
    updateStats();
  });
